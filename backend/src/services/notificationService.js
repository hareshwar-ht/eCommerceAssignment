...
const queueNotification = async ({ type, userId, recipient, templateName, payload }) => {
...
const processDirect = async (message) => {
  try {
    ...
    await notificationModel.updateHistoryStatus(message.historyId, 'failed', err.message);
    throw err;
  }
};
...
const processEmail = async (message) => {
  try {
    ...
    await notificationModel.updateHistoryStatus(message.historyId, status, err.message);
    throw err;
  }
};

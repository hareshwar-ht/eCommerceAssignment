...
describe('Notification Service', () => {
  ...
  describe('queueNotification', () => {
    it('should queue an email notification successfully', async () => {
      const template = { id: 'uuid', template_id: 'sg-temp-id', name: 'welcome' };
      ...
      expect(rabbitmq.publish).toHaveBeenCalledWith(rabbitmq.QUEUES.EMAIL, expect.objectContaining({
        type: 'email',
        recipient: 'test@example.com',
    ...
    it('should throw an error if template is not found', async () => {
      ...
      await expect(notificationService.queueNotification({
        type: 'email',
        userId: 'user-id',
        recipient: 'test@example.com',

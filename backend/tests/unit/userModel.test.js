...
describe('User Model', () => {
  ...
  describe('findByEmail', () => {
    it('should find user by email', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT id, name, email, phone, role, password_hash FROM users WHERE email ='),
        ['test@example.com']
      );
  ...
  describe('findById', () => {
    it('should find user by id', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT id, name, email, phone, role, created_at FROM users WHERE id ='),
        ['1']
      );
      expect(user).toEqual(mockUser);
    });
  ...
  describe('saveResetToken', () => {
    it('should save token and expiry', async () => {
      pool.query.mockResolvedValue({ rows: [] });
      await userModel.saveResetToken('bob@example.com', 'token-abc', new Date());
      expect(pool.query).toHaveBeenCalledWith(
  ...
  describe('findByResetToken', () => {
    it('should find user with valid token and expiry', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT id, name, email FROM users WHERE reset_token ='),
        ['token-abc']
      );
      expect(user).toEqual(mockUser);
    });

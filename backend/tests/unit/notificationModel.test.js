...
describe('Notification Model', () => {
  ...
  describe('createTemplate', () => {
    it('should insert a new notification template', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        ...
        ['test-template', 'email', 'sg-id', 'Welcome', JSON.stringify(['name'])]
      );
  ...
  describe('findByName', () => {
    it('should find active template by name', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM notification_templates WHERE name ='),
        ['welcome-email']
      );
  ...
  describe('findById', () => {
    it('should return joined notification history details', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT nh.*, nt.template_id as external_template_id'),
        ['hist-123']
      );
  ...
  describe('createHistory', () => {
    it('should insert history record', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        ...
        ['user-123', 'email', 'test@example.com', 'temp-123', JSON.stringify({ name: 'Alice' })]
      );
  ...
  describe('updateHistoryStatus', () => {
    it('should update status and error details', async () => {
      pool.query.mockResolvedValue({ rows: [] });
      await notificationModel.updateHistoryStatus('hist-123', 'failed', 'SMTP Error');
      expect(pool.query).toHaveBeenCalledWith(
  ...
  describe('getHistory', () => {
    ...
    it('should apply filters (userId, type, status)', async () => {
      ...
      await notificationModel.getHistory({ userId: 'u-1', type: 'email', status: 'sent', page: 1, limit: 10 });
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('WHERE user_id = $1 AND type = $2 AND status = $3'),
        ['u-1', 'email', 'sent', 10, 0]
      );
    });
  });

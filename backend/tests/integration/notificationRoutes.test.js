...
describe("Notification Routes Integration", () => {
  ...
  describe("POST /api/notifications/send", () => {
    it("should trigger a notification successfully", async () => {
      notificationService.queueNotification.mockResolvedValue({
        id: "hist-id",
      });
  ...
  describe("POST /api/notifications/templates", () => {
    it("should create template successfully", async () => {
      const templateData = {
        name: "order-confirm",
        type: "email",
        templateId: "sg-temp-123",
        subject: "Your Order",
      };
      notificationModel.findByName.mockResolvedValue(null);
      notificationModel.createTemplate.mockResolvedValue({
        id: "temp-uuid",
        ...templateData,
      });
  ...
  describe("GET /api/notifications/history", () => {
    it("should retrieve history logs", async () => {
      const logs = {
        records: [{ id: "1", recipient: "bob@example.com" }],
        total: 1,
        page: 1,
        limit: 20,
      };

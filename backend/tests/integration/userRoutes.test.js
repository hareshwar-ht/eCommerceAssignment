...
describe('User Routes / Auth Controller Integration', () => {
  ...
  beforeAll(() => {
    ...
    token = jwt.sign({ sub: 'user-123', role: 'user' }, accessSecret, { expiresIn: '1h' });
  });
  ...
  describe('POST /api/user/register', () => {
    it('should register a new user successfully', async () => {
      const userData = { name: 'Alice', email: 'alice@example.com', password: 'password123', phone: '9876543210' };
      const createdUser = { id: 'alice-id', name: 'Alice', email: 'alice@example.com', phone: '+919876543210', role: 'user' };
    ...
    it('should fail registration if email is already taken', async () => {
      ...
      const response = await request(app)
        .post('/api/user/register')
        .send({ name: 'Bob', email: 'bob@example.com', password: 'password123', phone: '9876543210' });
  ...
  describe('POST /api/user/register/initiate', () => {
    it('should initiate OTP registration successfully', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(notificationService.queueNotification).toHaveBeenCalledWith(expect.objectContaining({
        type: 'sms',
  ...
  describe('POST /api/user/login', () => {
    it('should login user and set cookie', async () => {
      const user = { id: 'user-id', email: 'user@example.com', password_hash: 'hash', role: 'user' };
  ...
  describe('POST /api/user/forgot-password', () => {
    it('should send reset URL', async () => {
      userModel.findByEmail.mockResolvedValue({ id: 'user-id', email: 'user@example.com' });
  ...
  describe('GET /api/user/profile', () => {
    it('should return profile for authenticated user', async () => {
      userModel.findById.mockResolvedValue({ id: 'user-123', name: 'Bob', email: 'bob@example.com' });
